...
const defaultProducts = [
  ...
  { id: 110, name: "Holbein Artists Gouache", category: "Cat Air", price: 620000, stock: 7, desc: "Cat gouache premium buatan Jepang bertekstur pekat pekat.", icon: "🎨" }
];
  ...
  grid.innerHTML = filtered.map(p => {
    return `
      <div class="product-card">
        <div class="pc-image-box">${p.icon}</div>
        <div class="pc-info">
          <div>
...
function addToCart(id) {
  ...
  if(inCart) {
    ...
    cart.push({ id: prod.id, name: prod.name, price: prod.price, icon: prod.icon, qty: 1 });
  }
  updateCartBadge();
  saveData();
...
function renderCartItems() {
  ...
  container.innerHTML = cart.map(item => `
    <div class="cart-item-row">
      <div class="cr-img">${item.icon}</div>
      <div class="cr-info">
        <h5>${item.name}</h5>
